...
# Mobile numbers for automated messaging
AUTOMATED_MESSAGE_MOBILES = ["8919947737", "6363735476", "9346132404", "9603947155"]
...
def init_db():
    ...
    # New table for automated messaging configuration
    cur.execute("""
    ...
    conn.commit()
    # seed demo data minimal (if empty) - Updated with the 4 mobile numbers
    cur.execute("SELECT COUNT(*) FROM students")
    if cur.fetchone()[0] == 0:
        students = [
            ("yagensh", "Yagensh", "/avatars/yagensh.jpg", 1, 1, "8919947737", "A"),
            ("saigireesh", "Sai Gireesh", "/avatars/saigireesh.jpg", 1, 2, "6363735476", "A"),
            ("venkat", "Venkat", "/avatars/venkat.jpg", 1, 3, "9346132404", "A"),
            ("hasini", "Hasini", "/avatars/hasini.jpg", 1, 4, "9603947155", "A")
        ]
    ...
    # Add mobile numbers to messaging config
    cur.execute("SELECT COUNT(*) FROM messaging_config")
    if cur.fetchone()[0] == 0:
        for mobile in AUTOMATED_MESSAGE_MOBILES:
            cur.execute("INSERT OR REPLACE INTO messaging_config (mobile_number, is_active, purpose) VALUES (?, 1, ?)",
                       (mobile, "Automated attendance notifications"))
    
    conn.commit()
    conn.close()
    print("[INFO] DB initialized with mobile numbers for automated messaging")
    print(f"[INFO] Configured mobile numbers: {AUTOMATED_MESSAGE_MOBILES}")
...
# ---------------------------
def get_automated_message_numbers():
    """Get all active mobile numbers for automated messaging"""
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT mobile_number FROM messaging_config WHERE is_active = 1")
    rows = cur.fetchall()
    conn.close()
    return [row[0] for row in rows]

def trigger_automated_message(student_id: str, status: str, mobile_number: str):
    """
    Placeholder function for triggering automated messages.
    Your friend can implement the actual messaging logic here.
    """
    try:
        conn = get_conn()
        cur = conn.cursor()
        ...
        if student:
            ...
            # Log the message trigger
            print(f"[AUTOMATED_MSG] Student: {student_name} ({student_id})")
            print(f"[AUTOMATED_MSG] Status: {status}")
            print(f"[AUTOMATED_MSG] Mobile: {mobile_number}")
            print(f"[AUTOMATED_MSG] Timestamp: {timestamp}")
            
            # Your friend can add actual SMS/messaging API calls here
            # Example message format:
            message = f"PresenceAI Alert: {student_name} marked as {status} at {timestamp}"
            print(f"[AUTOMATED_MSG] Message: {message}")
            
            return True
    except Exception as e:
        print(f"[ERROR] Failed to trigger automated message: {e}")
        return False
...
async def get_messaging_config(user = Depends(require_token)):
    """Get automated messaging configuration"""
    ...
    try:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("SELECT mobile_number, is_active, purpose, added_at FROM messaging_config ORDER BY added_at")
        rows = cur.fetchall()
        conn.close()
        ...
        return {"messaging_numbers": config}
    except Exception as e:
        print("[ERROR] get_messaging_config:", e)
        return {"messaging_numbers": []}

@app.post("/api/messaging/config")
async def update_messaging_config(payload: Dict[str, Any], user = Depends(require_token)):
    """Update automated messaging configuration"""
    if user["role"] not in ["hod", "teacher"]:
        raise HTTPException(status_code=403, detail="HOD or Teacher access required")
    
    mobile_number = payload.get("mobile_number")
    is_active = payload.get("is_active", True)
    purpose = payload.get("purpose", "Automated attendance notifications")
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            INSERT OR REPLACE INTO messaging_config (mobile_number, is_active, purpose)
            VALUES (?, ?, ?)
        """, (mobile_number, is_active, purpose))
        conn.commit()
        conn.close()
        
        return {"success": True, "message": "Messaging configuration updated"}
    except Exception as e:
        print("[ERROR] update_messaging_config:", e)
        raise HTTPException(status_code=500, detail="Failed to update messaging configuration")
...
async def checkin(files: List[UploadFile] = File(None)):
    ...
    # Get student's mobile number for automated messaging
    cur.execute("SELECT mobile FROM students WHERE id = ?", (student_id,))
    student_row = cur.fetchone()
    conn.close()
    
    # Trigger automated messages to configured numbers
    automated_numbers = get_automated_message_numbers()
    for mobile_num in automated_numbers:
        try:
            trigger_automated_message(student_id, "Present", mobile_num)
        except Exception as e:
            print(f"[WARN] Failed to send automated message to {mobile_num}: {e}")
    ...
    try:
        ...
        await manager.broadcast(json.dumps({
            ...
            "payload":{
                ...
                "automated_messages_sent": len(automated_numbers)
            }
        }))
    except Exception as e:
        ...
        while True:
            ...
            if "img" in msg:
                ...
                if best_dist <= WEBSOCKET_THRESHOLD:
                    ...
                    # Trigger automated messages for WebSocket checkins too
                    automated_numbers = get_automated_message_numbers()
                    for mobile_num in automated_numbers:
                        try:
                            trigger_automated_message(student_id, "Present", mobile_num)
...
# ---------------------------
# Helper: mark_student_present
# ---------------------------
def mark_student_present(student_id: str, attendance_date: date):
    """
    Marks the student as present for the given date in daily_attendance table.
    If no record exists, creates one. If already present, updates status.
    """
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO daily_attendance (student_id, attendance_date, status, checkin_time)
        VALUES (?, ?, 'present', ?)
        ON CONFLICT(student_id, attendance_date) DO UPDATE SET
            status='present',
            checkin_time=COALESCE(daily_attendance.checkin_time, CURRENT_TIMESTAMP),
            updated_at=CURRENT_TIMESTAMP
    """, (student_id, str(attendance_date), datetime.now()))
    conn.commit()
    conn.close()
...
# ---------------------------
# New endpoint: Test automated messaging
# ---------------------------
@app.post("/api/test-messaging")
async def test_messaging(payload: Dict[str, Any], user = Depends(require_token)):
    """Test automated messaging system"""
    if user["role"] not in ["hod", "teacher"]:
        raise HTTPException(status_code=403, detail="HOD or Teacher access required")
    
    student_id = payload.get("student_id", "test_student")
    ...
    for mobile_num in automated_numbers:
        try:
            if trigger_automated_message(student_id, status, mobile_num):
                successful_messages += 1
            else:
                failed_messages += 1
        except Exception as e:
            print(f"[ERROR] Test message failed for {mobile_num}: {e}")
            failed_messages += 1
    
    return {
        "success": True,
        "message": f"Test messages sent to {len(automated_numbers)} numbers",
        "successful_messages": successful_messages,
        "failed_messages": failed_messages,
        "configured_numbers": automated_numbers
    }
...
async def startup_event():
    ...
    print(f"[INFO] Configured automated messaging numbers: {AUTOMATED_MESSAGE_MOBILES}")
