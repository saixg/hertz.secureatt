...
def is_live_face(img_bytes: bytes) -> bool:
    """Return True if DeepFace thinks it's live, False if spoof/photo."""
    try:
...
async def checkin(files: List[UploadFile] = File(None)):
    ...
    for f in files:
        b = await f.read()
        # 🚫 Anti-spoofing check on each frame
        if not is_live_face(b):
            return JSONResponse({
                "success": False,
                "status": "suspicious",
                "message": "Spoof detected — please use live face"
            })
        frames.append(b)
    except:
            pass
    ...
    try:
        ...
        while True:
            ...
            if "img" in msg:
                ...
                # 🚫 Anti-spoofing check
                if not is_live_face(img_bytes):
                    await websocket.send_text(json.dumps({
                        "type": "suspicious",
                        "message": "Spoof/photo detected — only live faces allowed"
                    }))
                    continue
                except Exception:
