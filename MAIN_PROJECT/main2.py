...
# ------------------------
# API: checkin (multi-frame) - UPDATED WITH ENHANCED LIVENESS
# ------------------------
...
async def checkin(files: List[UploadFile] = File(None)):
    ...
    # Update daily attendance based on detection result
    try:
        if is_suspicious:
            # Mark as suspicious in daily attendance
            ...
            cur.execute(
                ...
                (student_id, "suspicious", "Suspicious checkin - possible spoof detected")
            )
            cur.execute("UPDATE trust_scores SET score = MAX(score - 10, 0), updated_at = CURRENT_TIMESTAMP WHERE student_id = ?", (student_id,))
            status_label = "Suspicious"
        else:
            # Mark as present in daily attendance
            ...
            cur.execute("UPDATE trust_scores SET score = MIN(score + 2, 100), streak = streak + 1, punctuality = MIN(punctuality + 1,100), updated_at = CURRENT_TIMESTAMP WHERE student_id = ?", (student_id,))
...
# ------------------------
# WebSocket endpoint - UPDATED WITH ENHANCED LIVENESS
# ------------------------
    ...
    try:
        ...
        while True:
            ...
            if "img" in msg:
                ...
                # Enhanced liveness check first
                if enhanced_liveness_detection(img_bytes):
                    await websocket.send_text(json.dumps({"type":"suspicious", "message":"Potential spoof detected - please ensure live face"}))
                    continue
                ...
                if best_dist <= TOL:
                    ...
                    # Update daily attendance when recognized via WebSocket
